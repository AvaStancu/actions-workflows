...
const (
	...
	ephemeralRunnerFinalizerName = "ephemeralrunner.actions.summerwind.net/finalizer"
)
...
// +kubebuilder:rbac:groups=actions.summerwind.dev,resources=ephemeralrunners/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=actions.summerwind.dev,resources=ephemeralrunners/finalizers,verbs=get;list;watch;create;update;patch;delete
...
func (r *EphemeralRunnerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !ephemeralRunner.ObjectMeta.DeletionTimestamp.IsZero() {
		if controllerutil.ContainsFinalizer(ephemeralRunner, ephemeralRunnerFinalizerName) {
			...
			if err != nil {
				log.Error(err, "Failed to finalize ephemeral runner")
	...
	if controllerutil.AddFinalizer(ephemeralRunner, ephemeralRunnerFinalizerName) {
		log.Info("Adding finalizer")
		if err := r.Update(ctx, ephemeralRunner); err != nil {
	...
	case cs.State.Terminated == nil: // still running
		log.Info("Ephemeral runner container is still running")
		if err := r.updateStatusFromPod(ctx, ephemeralRunner, pod, log); err != nil {
...
func (r *EphemeralRunnerReconciler) cleanupResources(ctx context.Context, ephemeralRunner *v1alpha1.EphemeralRunner, log logr.Logger) (deleted bool, err error) {
	...
	// TODO: Do we need to delete anything left by the container hooks?
...
func (r *EphemeralRunnerReconciler) updateStatusFromPod(ctx context.Context, ephemeralRunner *v1alpha1.EphemeralRunner, pod *corev1.Pod, log logr.Logger) error {
