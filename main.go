...
func main() {
	...
	flag.BoolVar(&disableAdmissionWebhook, "disable-admission-webhook", false, "Disable admission webhook for controller manager.")
	...
	// AutoRunnerScaleSet will not need to have a shared GitHubClient
	...
	actionsMultiClient := actions.NewMultiClient(
		...
		logger.WithName("actions-clients"),
	)
	...
	if err = (&controllers.EphemeralRunnerReconciler{
		Client:        mgr.GetClient(),
	...
	if err = (&controllers.EphemeralRunnerSetReconciler{
		Client:        mgr.GetClient(),
	...
	if err = (&controllers.AutoscalingListenerReconciler{
		Client: mgr.GetClient(),
