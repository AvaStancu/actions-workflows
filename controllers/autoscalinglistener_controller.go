...
import (
	...
	v1alpha1 "github.com/actions-runner-controller/actions-runner-controller/api/v1alpha1"
...
const (
	...
	autoscalingListenerFinalizerName = "autoscalinglistener.actions.summerwind.dev/finalizer"
)
...
// +kubebuilder:rbac:groups=actions.summerwind.dev,resources=autoscalinglisteners/finalizers,verbs=update
...
func (r *AutoscalingListenerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if controllerutil.AddFinalizer(autoscalingListener, autoscalingListenerFinalizerName) {
		...
		if err := r.Update(ctx, autoscalingListener); err != nil {
	...
	// TODO: make sure the mirror secret is up to date
	...
	desiredRulesHash := ComputeHash(&desiredRules)
...
func (r *AutoscalingListenerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&corev1.Secret{}).
		Watches(&source.Kind{Type: &corev1.Secret{}}, handler.EnqueueRequestsFromMapFunc(labelBasedWatchFunc)).
