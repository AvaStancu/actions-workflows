...
// Init sets up and return the *github.Client for the object.
func (c *MultiGitHubClient) InitForEphemeralRunner(ctx context.Context, r *v1alpha1.EphemeralRunner) (*github.Client, error) {
	// if len(r.Spec.GitHubAuthSecret) == 0 {
	// 	return nil, fmt.Errorf("GitHubAuthSecret is required creating github api client.")
	// }

	ref := refFromEphemeralRunner(r)
	if ref.ns != r.Namespace {
		return nil, fmt.Errorf("referencing github api creds secret from owner in another namespace is not supported yet")
	}

	// kind can be any of Runner, RunnerReplicaSet, or RunnerDeployment depending on which custom resource the user directly created.
	return c.initClientWithSecretName(ctx, r.Namespace, r.Spec.GitHubConfigSecret, ref)
}
...
func refFromEphemeralRunner(r *v1alpha1.EphemeralRunner) *runnerOwnerRef {
	return &runnerOwnerRef{
		kind: r.Kind,
		ns:   r.Namespace,
		name: r.Name,
